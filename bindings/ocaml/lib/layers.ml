...
let new_retry_layer ?(factor: float option) ?(jitter:bool = false)
    ?(max_delay : Time.Span.t option )
    ?(min_delay : Time.Span.t option ) max_times =
...
let new_timeout_layer ?(speed:int64 option) timeout =
  let timeout = Time.Span.to_ms timeout in 
  Opendal_core.Layers.new_timeout_layer timeout speed

let new_logging_layer f = 
  Opendal_core.Layers.new_logging_layer f